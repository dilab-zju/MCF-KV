...
#ifndef SB_KV_FF_BTREEE_H
#define SB_KV_FF_BTREEE_H
...
//      Entry* array_end = &records[1];
            ...
            bool inserted = false;
            // what dose this mean? do not understand
            ...
            for (i = *num_entries - 1; i >= 0; i--) {
                if (key < records[i].key) {
                    ...
                    if (flush) {
                        ...
                        uint64_t remainder = records_ptr % CACHE_LINE_SIZE;
                        //reach the cache_line_size or
                        //over a cache_line_size
                        bool do_flush = (remainder == 0) ||
                                        ((((int) (remainder + sizeof(Entry)) / CACHE_LINE_SIZE) == 1)
                                         && ((remainder + sizeof(Entry)) % CACHE_LINE_SIZE) != 0);
                        if (do_flush) {
                            clflush((char*) records_ptr, CACHE_LINE_SIZE);
                            memory_cost += CACHE_LINE_SIZE;
                            to_flush_cnt = 0;
                        } else
                            ++to_flush_cnt;
                    }
