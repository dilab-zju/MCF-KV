...
DBImpl::~DBImpl() {
  ...
  assert(pm_root_->index!= nullptr);
  // why this memory is gone?
...
void DBImpl::CompactMemTable() {
  ...
  if (s.ok()) {
    ...
    s = versions_->LogAndApply(&edit, &mutex_, false);
  }
...
void DBImpl::BackgroundCompaction() {
  ...
  if (imm_ != nullptr) {
    CompactMemTable();
//    versions_->CheckIfTooManyFile();
//    versions_->PickCandidates();
//    versions_->CheckLocality();
    return;
  }
...
Status DBImpl::FinishCompactionOutputFile(CompactionState* compact,
  ...
  if (s.ok()) {
      // here finish is called by compaction thread
    s = compact->builder->Finish(compact->compaction->edit(), true);
  } else {
    compact->builder->Abandon();
  }
...
Status DBImpl::InstallCompactionResults(CompactionState* compact) {
  ...
  return versions_->LogAndApply(compact->compaction->edit(), &mutex_, true);
}
...
#endif
    assert(pm_root_->index!= nullptr);
    uint32_t m_ = pm_root_->index->Get(ExtractUserKey(key));
    assert(m_ != -1);
    // Check validation
    if (!compact->compaction->IsInput(m_)) {
      drop = true;
    }
...
//    }
  ...
  if (s.ok() && save_manifest) {
    ...
    s = impl->versions_->LogAndApply(&edit, &impl->mutex_, false);
  }
  if (s.ok()) {
...
DBImpl::PM_root* DBImpl::allocate_pm_root(Index* index_) {
    PM_root* p = static_cast<PM_root*>(malloc(sizeof(PM_root)));
