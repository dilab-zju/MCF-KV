...
class Version {
 public:
  ...
  Status Get(const ReadOptions&, const LookupKey& key, std::string* val, uint32_t*);
  ...
  void SetFileCheckCount(std::shared_ptr<FileMetaData> f, int initial_count);
  ...
  bool MoveToMerge(std::set<uint32_t> array, bool is_scan);
 ...
 private:
  ...
  std::unordered_map<uint64_t, int> file_check_count_;
  std::vector<std::shared_ptr<FileMetaData>> files_to_compact_;
