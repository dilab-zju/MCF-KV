...
class VersionControl {
 public:
  ...
  Status LogAndApply(VersionEdit* edit, port::Mutex* mu, bool major);
  Compaction* PickCompaction();
  void RegisterFileAccess(const uint16_t& file_number);
  void CheckLocality();
  void PickCandidates(Version* v);
  // Evict merge candidates if pick compaction failed.
  void EvictCandidates(Version* v);
  void EvictAll(Version* v);
