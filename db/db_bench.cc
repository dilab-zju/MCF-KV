...
static int FLAGS_num = 50 * 1000 * 16;
...
// Size of each value
static int FLAGS_value_size = 1024 *  4;
...
static bool FLAGS_csv = true;

// CSV file
static FILE* csv_file = fopen("/home/zhj/SLM-DB/YCSB_res.csv","rw");

// Number of bytes to buffer in memtable before compacting
// (initialized to default value by "main")
static int FLAGS_write_buffer_size = 1024 * 1024 * 64;
...
// Range query size
static int FLAGS_range_size = 1000000;

// Use the db with the following name.
static const char* FLAGS_db = NULL;

// trace dir
static std::string FLAGS_trace = "/media/nvme1/trace";
static bool FLAGS_ycsb = true;
...
private:
  ...
  void Open() {
    ...
    std::cout<<"max file size:" << (options.max_file_size >> 20) << " MB"<< std::endl;
    options.block_size = FLAGS_block_size;
    options.max_open_files = FLAGS_open_files;
    options.filter_policy = filter_policy_;
    options.reuse_logs = FLAGS_reuse_logs;
    options.merge_threshold = FLAGS_merge_threshold;
    options.index = CreateMCFIndex();
...
//        uint64_t bytes = 0;
//        reads_ /= 10;
//        uint64_t k = thread->rand.Next() % FLAGS_num;
...
//  }
  ...
  void RangeQuery(ThreadState* thread) {
    ...
    reads_ = num_ / 10;
    int found = 0;
    uint64_t k = (thread->rand.Next() % FLAGS_num) - reads_;
    for (int i = 0; i < reads_; ++i) {
        char key[100];
        snprintf(key, sizeof(key), config::key_format, k++);
        if (db_->Get(options, key, &value).ok()) {
